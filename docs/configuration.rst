...
The structure of this configuration file is documented below. Most of the fields in the ``package`` block resemble the structure of a ``setup.py`` file, while the ``documentation`` block collects additional settings for the Sphinx configuration residing in the ``conf.py`` file in the ``docs`` subdirectory of the project.
